...
# تمیز کردن حافظه موقت برای جلوگیری از مشکلات فضای دیسک
echo "Cleaning temporary files..."
rm -rf /data/data/com.termux/files/usr/tmp/*

# نصب پیش‌نیازها
echo "Installing requirements..."
pkg update && pkg upgrade -y || { echo "Failed to update packages"; exit 1; }
pkg install git python python-pip figlet libmd -y || { echo "Failed to install packages"; exit 1; }
pip install --no-build-isolation setuptools==68.2.2 || { echo "Failed to install setuptools"; exit 1; }
pip install --no-build-isolation opencv-python-headless==4.8.1.78 || { echo "Failed to install opencv-python-headless"; exit 1; }
pip install --no-build-isolation stegano==0.10.1 || { echo "Failed to install stegano"; exit 1; }
